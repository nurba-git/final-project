...
/**
 * SOLID-O: абстрактный базовый. Новый враг = новый подкласс.
 * SOLID-L: все подклассы взаимозаменяемы везде, где ожидается Enemy.
 */
public abstract class Enemy implements IDamageable, IUpdatable {
    ...
    public String type; // для QuestManager.notifyKill(type)
    ...
    /** HP бар и базовое обновление hitFlash — вызывается из каждого draw(). */
    ...
    /** Базовый простой круговой рендер, переопределяется в подклассах. */
    @Override
