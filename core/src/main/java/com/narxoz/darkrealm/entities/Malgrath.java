...
public class Malgrath extends Enemy {
    ...
    public Malgrath(float x,float y, CollisionSystem col, BulletPool bp, ParticleSystem fx){
        super(x,y,500,30,1.0f*60f,16f,ARMOR,col,bp,fx);
    ...
    @Override public void update(float dt) {
        ...
        // Melee contact
        if (d < radius+C.PLAYER_RADIUS+3 && !target.isInvincible()) target.takeDamage(damage);

        // Phase 1 — void slash
        if (slashTimer<=0) {
            int dmg = phase==Phase.THREE ? (int)(damage*1.5f) : damage;
            if (d<C.SHADOW_STRIKE_RANGE+radius) { target.takeDamage(dmg); slashTimer=1.8f; }
        }
